...
Dacă `str1` este egal cu `LEET` și `str2` este egal cu `CODE`, atunci `str1 + str2` nu este egal cu `str2 + str1`, deci răspunsul este un șir gol.

Și, dacă `str1` este egal cu `ababab` și `str2` este egal cu `abab`, atunci `str1+str2` este egal cu `str2+str1`. Următorul pas este să calculez cel mai mare divizor comun, `gcd(6,4)`, care este `2`. Așadar, răspunsul este subșirul lui `str1` de la poziția `0` până la `2`, adică `ab`.
